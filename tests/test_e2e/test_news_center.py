...
class TestNewsCenterDispatch:
    async def test_default_sub_agents_are_registered(self):
    ...
    async def test_aggregator_returns_all_4_components(self):
        news = get_agent("news_center")
        assert isinstance(news, NewsCenterAgent)
        result = await news.run(_ctx("AI 监管"))
    ...
    async def test_aggregator_view_shape(self):
        news = get_agent("news_center")
        result = await news.run(_ctx("AI 监管"))
    ...
    async def test_trace_records_per_subagent_step(self):
        news = get_agent("news_center")
        result = await news.run(_ctx())
    ...
    async def test_isolates_per_subagent_failures(self):
    ...
    async def test_partial_failure_yields_partial_data(self):
    ...
    async def test_warnings_collect_from_all_subagents(self):
    ...
    async def test_memory_keys_present_in_data(self):
        result = await get_agent("news_center").run(_ctx("topic-xyz"))

...
class NarrativePipeline:
    ...
    三阶段分析：
    1. build_global_summary — 全局局势总结（中文，2-4段）
    2. build_narrative_clusters — 按叙事主题聚类（返回 JSON）
    3. build_tension_matrix — 行为体之间的张力关系矩阵（返回 JSON）

    所有 LLM 调用均受异常保护，失败时返回降级结果而非抛异常。
    """
    ...
    def build_global_summary(self, articles: List[Dict[str, Any]]) -> str:
        ...
        Args:
            articles: 新闻字典列表，每个含 title, summary, source
        ...
        try:
            ...
            logger.error(f"[NarrativePipeline] 全局总结失败: {e}")
            return f"（全局总结生成失败：{e}）"
    ...
    def run(
        ...
        Args:
            articles: 新闻字典列表，每项含 title, summary, source 等字段。
                      如果传入字符串，将作为主题进行简化分析。

        Returns:
            ServiceResult:
            - success=True: data 含 global_summary, clusters, tension_matrix
            - success=False: error 含错误描述
        """
        ...
        logger.info(f"[NarrativePipeline] 开始叙事分析，共 {len(articles)} 条")
        ...
        try:
            ...
            logger.info("[NarrativePipeline] 叙事分析完成")
            ...
            logger.error(f"[NarrativePipeline] 分析失败: {e}", exc_info=True)
            return ServiceResult.fail(f"叙事分析失败: {e}")
